            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = f'[B][C]{get_random_color()}\n- WeLComE To Emote Bot !\n - Emote command :\n- parafal\n- mp40\n- ak47\n- m1014\n- m1887\n- lv100\n\n{get_random_color()}- Following TikTok : Senzu.!'
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip().startswith('@qa'):
                            ...
                            try:
                                ...
                                try:
                                    uid = int(parts[1])
                                    idT = emote_list.get(uid)
                                    if idT is None:
                                        idT = int(uid)
                                    for p in parts[2:]:
                                        uid_list.append(int(p))
                        ...
                        if inPuTMsG.strip().startswith('@a'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                print("Thiếu teamcode, preset hoặc UID!")
                                return
                            ...
                            try:
                                idT = emote_presets.get(preset_name)
                                if idT is None:
                                    idT = int(preset_name)
                            except Exception as e:
                                print("Lỗi preset:", e)
                            ...
                            try:
                                H1 = await Emote_k(uid, idT, key, iv, region)
                                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H1)
                        
                                await asyncio.sleep(0.3)
                                H2 = await Emote_k(target_uid, idT, key, iv, region)
                                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H2)
                        
                                print(f"Kích hoạt emote {preset_name} ({idT}) cho UID {target_uid}")
                            except Exception as e:
                                print("Emote error:", e)
...
async def MaiiiinE():
    Uid , Pw = '4236374729','BY_PARAHEX-U2TRZD25P-REDZED'
